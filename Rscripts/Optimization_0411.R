...
P4 = problem(PUsT_Opt, features = SpsStack1[[47:222]], cost_column = "AgriRent") %>%
  add_min_set_objective() %>%
...
#Setting up diffrent penalty values of not protecting CO2 emission
...
#Below: Maximum features objective
penaltiesVec=c(0.001,0.0025,0.005,0.0075,0.01)
spsCovVec = c()
co2vec2 = c()
for(i in 1:5){
  p8 <- p5 %>% 
  ...
  s8 <- solve(p8)
  tc_s5 <- eval_target_coverage_summary(p8, s8[, "solution_1"])
  co2vec2[i] = sum(data.frame(s8[, "solution_1"])*(PUs4$CO2))
  spsCovVec[i] = sum(tc_s5$met)
}
